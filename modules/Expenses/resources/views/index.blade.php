...
@section('content')
    <h1>{{ __('Expenses') }}</h1>
    <p class="sub">{{ $organization?->name }} — Money going out: expense claims against accounts, with receipts and approval.</p>

    <div class="grid c3">
        <a class="stat" href="{{ route('expenses.records.index') }}">
            <div class="n">{{ number_format($count) }}</div>
            <div class="k">{{ __('Expenses') }}</div>
        </a>
    ...
    <div class="card table-wrap" style="margin-top:16px">
            ...
            @forelse ($recent as $record)
                <tr>
                    @foreach ($columns as $attribute => $label)
                        <td>
                            @if ($loop->first)
                                <a href="{{ route('expenses.records.edit', $record) }}">{{ \App\Support\Present::cell($record, $attribute, []) }}</a>
                            @else
        ...
        <p class="small"><a href="{{ route('expenses.records.index') }}">{{ __('All Expenses') }}</a></p>
    </div>
@endsection
