...
@section('content')
    <h1>{{ __('Projects') }}</h1>
    <p class="sub">{{ $organization?->name }} — Projects, their tasks and the hours logged against them.</p>

    <div class="grid c3">
        <a class="stat" href="{{ route('projects.records.index') }}">
            <div class="n">{{ number_format($count) }}</div>
            <div class="k">{{ __('Projects') }}</div>
        </a>
    ...
    <div class="card table-wrap" style="margin-top:16px">
            ...
            @forelse ($recent as $record)
                <tr>
                    @foreach ($columns as $attribute => $label)
                        <td>
                            @if ($loop->first)
                                <a href="{{ route('projects.records.edit', $record) }}">{{ \App\Support\Present::cell($record, $attribute, []) }}</a>
                            @else
        ...
        <p class="small"><a href="{{ route('projects.records.index') }}">{{ __('All Projects') }}</a></p>
    </div>
@endsection
