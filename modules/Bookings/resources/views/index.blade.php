...
@section('content')
    <h1>{{ __('Bookings') }}</h1>
    <p class="sub">{{ $organization?->name }} — Services, staff availability and appointments.</p>

    <div class="grid c3">
        <a class="stat" href="{{ route('bookings.records.index') }}">
            <div class="n">{{ number_format($count) }}</div>
            <div class="k">{{ __('Appointments') }}</div>
        </a>
    ...
    <div class="card table-wrap" style="margin-top:16px">
            ...
            @forelse ($recent as $record)
                <tr>
                    @foreach ($columns as $attribute => $label)
                        <td>
                            @if ($loop->first)
                                <a href="{{ route('bookings.records.edit', $record) }}">{{ \App\Support\Present::cell($record, $attribute, []) }}</a>
                            @else
        ...
        <p class="small"><a href="{{ route('bookings.records.index') }}">{{ __('All Appointments') }}</a></p>
    </div>
@endsection
